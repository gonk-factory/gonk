...
(triage-only, local rungs only, zero cloud budget), (b) a rendered explanation
of what gonk is, every configurable key, and the exact consequences of merging,
re-rendered from the actual config values so text cannot drift from settings, and
(c) a `.agent/` seed. Closing unmerged = declined; recorded; not re-opened unless
re-invited. If the bot lacks Developer, it opens an issue stating the role it
needs instead.

**`.agent/` ships WITH the onboarding MR, and it is context rather than a gate.**
It used to be a second, post-merge MR written by a metered agent session, and
until it landed the project sat at `pending` and dispatched nothing. That made a
model call the precondition for a project's first triage — the most expensive
possible way to start, and one that failed outright when the session had no
checkout to read. The seed is now rendered by the same deterministic, zero-token
template machinery as the `.gonk.yml` explanation, from the same config values,
and arrives in the same commit: `.agent/README.md` (what the directory is for and
the two ways to fill it in) plus skeleton files carrying the expected headings and
an explicit `NOT FILLED IN YET` marker per section.

Consequences, and they are the point:

- **Triage no longer waits.** A project with a merged `.gonk.yml` and no `.agent/`
  dispatches triage. `.agent/` makes the answer specific to the repository; its
  absence makes the answer thinner, not absent.
- **`pending` survives as a description.** It still means "no `.agent/` yet", it is
  still classified and still a metric label — it just no longer withholds work.
- **The metered scaffold is opt-in and out of v1.** `actions.scaffold` (schema v1,
  additive, default **false**) gates it. The project-scoped trigger and its
  Gate-1 path remain; with the default they are dormant. Turning it on is how a
  maintainer asks gonk to read the repository and propose `.agent/` content
  instead of writing it by hand, and it is gated behind the `file` effect kind
  and workflow-scope gating (ADR-007 §6, `gonk-066`).
- **The prompt reads it.** A triage session is given `.agent/` — the seeded paths,
  in order — ahead of the issue body. With no `.agent/`, the prompt is byte for
  byte the one it was before. This is the v1 thin loader: a fixed list of the
  paths gonk itself seeds, not a tree walk.

See ADR-007 §5: Navigator is not removed. Its conventions-plus-lint half is the
orientation tier, and `.agent/` remains its project-side home.
...
enabled: true
actions: { triage: true, pipelines: false, features: false, scaffold: false }
